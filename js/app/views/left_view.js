define(['jquery', 'underscore', 'backbone', '../routes/leftNavRouter', 'echarts', 'json!../data/bus-line.json', 'json!../data/hz-tracks.json', 'json!../data/relation.json', 'bmap', 'mapext'], 
function($, _, Backbone, leftRouter, ec , busline, tracks, relationData){
	var headerView = Backbone.View.extend({
		...
		render: function(){
			var self = this;
			var router = new leftRouter({routes:{
				""              : "scatterChart",
			...
			// 路线图
			router.on("route:lineChart", function(){
				var chart = ec.init($(".chart-list-panel")[0]);
				var hStep = 300 / (busline.length - 1);
				var busLines = [].concat.apply([], busline.map(function(busLine, idx) {
					var prevPt;
					var points = [];
					...
					return {
						...
						lineStyle: {
							normal: {
								color: ec.color.modifyHSL('#5A94DF', Math.round(hStep * idx))
							}
						}
					};
				}));
			...
			// 饼图
			router.on("route:pieChart", function(){
				var chart = ec.init($(".chart-list-panel")[0]);
				var data = [
					{
						name: '海门',
			...
			// 流向图
			router.on("route:flowLines", function(){
				var chart = ec.init($(".chart-list-panel")[0]);
				var geoCoordMap = {
				    '上海': [121.4648,31.2891],
				    '东莞': [113.8953,22.901],
			...
			// 柱状图
			router.on("route:barChart", function(){
				var chart = ec.init($(".chart-list-panel")[0]);
				var geoCoordMap = {
				    "海门":[121.15,31.89],
				    "鄂尔多斯":[109.781327,39.608266],
			...
			// 关系图
			router.on("route:relationChart", function(){
				var chart = ec.init($(".chart-list-panel")[0]);
				var geoCoordMap = {
				    "海门":[121.15,31.89],
				    "鄂尔多斯":[109.781327,39.608266],
