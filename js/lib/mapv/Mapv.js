...
var util = {
    isPlainObject: function isPlainObject(obj) {
        ...
        // Make sure that DOM ndes and window objects don't pass through, as well
...
function Layer(options) {
    ...
    this._drawer = {};
    this._eventHander = {
        click:function(item){console.log(item)},
        mouseover:function(item){console.log(item)},
...
util.extend(Layer.prototype, {
    initialize: function initialize() {
        ...
        var canvas = this.canvasLayer.getContainer();
        this.setCtx(canvas.getContext(this.getContext()));
    ...
    handlerEvent:function(item, canvas, type){
        if(type == 'click'){
            this._eventHander['click'](item, canvas);
        }else if(type == 'mouseover'){
             this._eventHander['mouseover'](item, canvas);
        }else if(type == 'mouseout'){
            this._eventHander['mouseout'](item, canvas);
        }
    },
    handlerEventConfig:function(obj){
        this._eventHander = obj;
    }
});
/**
...
SimpleDrawer.prototype.drawMap = function (time) {
    ...
    if (dataType === 'polyline') {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            var self = this;
            // 按级别分组画图
            var icons = drawOptions.icons;
            if(!!this.getLayer().images){
                var images = this.getLayer().images
            }else{
                var images = [];
                for(var i=0;i<icons.length;i++){
                    var imgn = new Image();
                    imgn.src = icons[i].url;
                    images.push(imgn);
                }
                this.getLayer().images = images;
            }
            var objectType = this.getLayer().objectType;
            window.cancelAnimationFrame(idAnimate);
            if(objectType == "event" || objectType == "clue"){
                var id = this.drawCustAnimation();
            }else if(objectType == "subject"){
                this.drawPointCtr(ctx, data, radius, images, icons);
            }else{
                this.drawPointCtr(ctx, data, radius, images, icons);
            }
            ...
            canvas.onclick = function(e){
                ...
                if(objectType == "event" || objectType == "clue"){
                    var isSelect = self.drawCustAnimationEvt(ctx, data, {x:pointx,y:pointy});
                }else{
                    var isSelect = self.drawPointCtr(ctx, data, radius, images, drawOptions.icons, {x:pointx,y:pointy}, 'click');
                }
                if(isSelect.length>0){
                    //处理点击事件
                    self.getLayer().handlerEvent(isSelect.pop(), canvas, 'click');
                }
            }
            canvas.onmousemove = function(e){
                ...
                if(objectType == "event" || objectType == "clue"){
                    ...
                    var isSelect = self.drawPointCtr(ctx, data, radius, images, drawOptions.icons, {x:pointx,y:pointy}, 'mouseover');
                }
                if(isSelect.length>0){
                    self.getLayer().handlerEvent(isSelect.pop(), canvas, 'mouseover');
                }else{
                    self.getLayer().handlerEvent(null, canvas, 'mouseout');
                    canvas.style.cursor = map.getDefaultCursor();
                }
            }
        }
    this.endDrawMap();
};
SimpleDrawer.prototype.drawCustAnimationEvt = function(ctx, data, point, type){
    var isSelect = [];
    for(var i=0,len=data.length;i<len;i++){
        var item = data[i];
        if(point.x > item.px - 30 && point.x < item.px + 30 && point.y > item.py -30 && point.y < item.py + 30){
            isSelect.push({item : item});
        }
     }
     return isSelect;
}
SimpleDrawer.prototype.drawCustAnimation = function(){
    var color = ['rgba(193,46,52)','rgb(230,182,0)','rgb(0,152,217)','rgb(43,130,29)','rgb(0,94,170)'];
    var data = this.getLayer().getData();
    var ctx = this.getCtx();
    for(var i=0, len=data.length; i<len; i++){
        var item = data[i];
        ctx.clearRect(item.px-50, item.py-50, 100,100);
    }
    for(var i=0, len=data.length; i<len; i++){
        var item = data[i];
        //ctx.clearRect(item.px-40, item.py-40, 80,80);
        ctx.beginPath();
        ctx.arc(item.px, item.py, item.influence*2, 0, 2*Math.PI, false);
        ctx.closePath();
        ctx.fillStyle = 'rgb(230,182,0)';
        ctx.fill();
        ctx.font = 12 + 'px serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = '#000';
        ctx.fillText(item.count, item.px, item.py)
        var grd=ctx.createRadialGradient(item.px, item.py, 10, item.px, item.py, 30);
        grd.addColorStop(0, 'rgba(230, 182, 0, 1)');
        grd.addColorStop(0.5, 'rgba(0, 152, 217, 0.6)');
        grd.addColorStop(1, 'rgba(43, 130, 29, 0.1)');
        ctx.strokeStyle = grd;
        ctx.lineWidth=2;
        var influence = item.influence || 5;
        for(var m=1;m<=3;m++){
            ctx.beginPath();
            ctx.arc(item.px, item.py, influence*2*m+step/10, 0, 2*Math.PI, false);
            ctx.closePath();
            ctx.stroke();
        }
    }
    ...
    var self = this;
    idAnimate = requestAnimationFrame(function(){self.drawCustAnimation();});
}
SimpleDrawer.prototype.drawPointCtr = function(ctx, data, radius, images, icons, point, type){
    ...
    for (var i = 0, len = data.length; i < len; i++) {
        ...
        var flag = (item.areaLevel == 6 && item.count > 1);
        if (!flag && images && images.length>0) {
            var flag = this.drawIcon(ctx, item, images, icons, point, type);
        } else {
            var flag = this.drawPoint(ctx, item, radius, point, type);
        }
        if(!!flag){
            isSelect.push(flag);
        }
...
*/
SimpleDrawer.prototype.drawPoint = function(ctx, item, radius, point, type){
    var radius =  7+Math.log(+item.count)*3;
    if(radius < 7){
        radius = 7;
    }
    if(type != "click" && type != "mouseover"){
        ctx.fillStyle = this.getDrawOptions().fillStyle;
        var color = ['rgb(193,46,52)','rgb(230,182,0)','rgb(0,152,217)','rgb(43,130,29)','rgb(0,94,170)'];
        var start = 0;
        if(!item['detail']){
            ctx.beginPath();
            ctx.arc(item.px, item.py, radius*1.2, 0 , 2*Math.PI, false);
            ctx.closePath();
            ctx.fillStyle = 'rgba(193, 46, 52, .8)';
            ctx.fill();
            ctx.fillStyle = '#fff';
        }else{
            for(var i=0;i<item['detail'].length;i++){
                var single = item['detail'][i];
                ctx.beginPath();
                ctx.lineTo(item.px, item.py);
                var ratio = single.count/item.count;
                ctx.arc(item.px, item.py, radius*1.2, start, ratio*2*Math.PI + start, false);
                start = ratio*2*Math.PI + start;
                ctx.closePath();
                ctx.fillStyle = color[single['typeNo']-1];
                ctx.fill();
            }
            ctx.beginPath();
	        ctx.arc(item.px, item.py, radius*0.8, 0, 2*Math.PI, false);
	        ctx.closePath();
	        ctx.fillStyle = "#fff";
	        ctx.fill();
	        ctx.fillStyle = '#000';
        ...
        */
        ...
        ctx.fillText(item.count, item.px, item.py);
        ctx.closePath();
        if (this.getDrawOptions().strokeStyle) {
            ctx.stroke();
        }
         ctx.restore();
    }
    var flag;
    if(this.get("layer").getDrawType() != "simple")return;
    if(point){
        var px = +item.px,
            py = +item.py,
            x = +point.x,
            y = +point.y;
        flag = Math.sqrt((px-x)*(px-x)+(py-y)*(py-y))+1 <= radius*1.3;
    }
    if(!!flag){
        return {item:item};
    }else{
...
// 绘制icon
SimpleDrawer.prototype.drawIcon = function (ctx, item, images, icons, point, type) {
    if(+item.areaLevel == 6){
        if(item.status == 0){
            var image = images[6],icon = icons[6];
        }else if(item.status == 1){
            var image = images[8],icon = icons[8];
        }else if(item.status == 2){
            var image = images[7],icon = icons[7];
        }else{
            var image = images[0],icon = icons[0];
        }
        if(item.detail){
            var index = item.detail[0]['typeNo'];
            if(index == 1){
                var image = images[9],icon = icons[9];
            }else if(index == 2){
                var image = images[10],icon = icons[10];
            }else if(index == 3){
                var image = images[11],icon = icons[11];
            }else if(index == 4){
                var image = images[12],icon = icons[12];
            }else if(index == 5){
                var image = images[13],icon = icons[13];
            }
        }
    }else if(+item.areaLevel == 5){
        var image = images[1],icon = icons[1];
    }else if(+item.areaLevel == 4){
        var image = images[2], icon = icons[2];
    }else if(+item.areaLevel == 3){
        var image = images[3], icon = icons[3];
    }else if(+item.areaLevel == 2){
        var image = images[4], icon = icons[4];
    }else if(+item.areaLevel == 1){
        var image = images[5], icon = icons[5];
    }else{
        var image = images[0],icon = icons[0];
    }
    var sx = icon.sx || 0,
        ...
        px = icon.px || 0,
        py = icon.py || 0,
        swidth = icon.swidth || 0,
        sheight = icon.sheight || 0,
        width = icon.width || 0,
        height = icon.height || 0;
    if(type != "click" && type != "mouseover"){
        //var image = new Image();
        // 单个个体分类的情况
        if((+item.areaLevel == 6 || item.count == 1) || (item['count'] > 1 && !item['detail'])){
            (function (item, sx, sy, swidth, sheight, width, height) {
                ...
                if(+item.areaLevel<6){
                    ...
                    ctx.textBaseline = 'middle';
                    ctx.fillStyle = '#fff';
                    ctx.fillText(item.count, item.px, item.py-py);
                }
                ctx.restore();
                //};
            ...
            var rectArr = [];
            var color = ['#c12e34','#e6b600','#0098d9','#2b821d','#005eaa'];
            if(item['detail']){
                for(var i=0;i<item['detail'].length;i++){
                    ctx.fillStyle = color[item['detail'][i]['typeNo']-1];
                    var offset = 100*item['detail'][i].count/item.count;
                    var x_pos = item.px-5-(7.5*item['detail'].length-1) + 15*i;
                    rectArr.push(offset);
                    rectArr.push(x_pos);
                    ctx.fillRect(x_pos, item.py-offset, 10, offset);
                }
            }/*else{
                var radius = height/2;
                if(item.count > height/2){
                    radius = height/2+item.count%(height/2);
                }else{
                    radius = height;
                }
                ctx.beginPath();
        ...
        for(var i=1;i<=5;i++){
            ctx.beginPath();
            ctx.lineTo(item.px, item.py);
            ctx.arc(item.px, item.py, i*height/5, (i-1)*2*Math.PI/5, i*2*Math.PI/5);
    ...
    if(point){
        var px = +(item.px - px),
            ...
            x = +point.x,
            y = +point.y;
        if(item['count']==1 || (item['count'] > 1 && !item['detail'])){
            flag = Math.abs(x-px) < +width/2 && Math.abs(y-py) < +height/2;
        }/*else if(item['count'] > 1 && !item['detail']){
            var radius = height/2;
            if(item.count > height/2){
                radius = height/2+item.count%(height/2);
            }else{
            ...
            flag = Math.abs(x-item.px) < radius && Math.abs(y-item.py) < radius;
        }*/else{
            for(var m=0;m<item['detail'].length;m++){
                var offset = 100*item['detail'][m].count/item.count;
                var x_pos = item.px-5-(7.5*item['detail'].length-1) + 15*m;
                if(y>=item.py-offset && y<=item.py && x>=x_pos && x<=x_pos+10){
                    flag = true;
                    break;
                }
            }
